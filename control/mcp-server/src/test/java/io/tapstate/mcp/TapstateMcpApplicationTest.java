...
class TapstateMcpApplicationTest {
    ...
    void startsAsAStdioOnlyApplicationAndRegistersTheConfiguredToolSurface() {
        ...
        try (ConfigurableApplicationContext context = TapstateMcpApplication.start(
            ...
            // The whole write surface: twelve reads plus six writes. Pinned as a count here and by name in
            // McpToolCatalogTest, so a tool that appears by accident fails one of the two.
            assertThat((List<?>) context.getBean("mcpTools")).hasSize(18);
        }
    }
}
